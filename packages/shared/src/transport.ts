...
export function createServerApp(
  registerTools: (server: McpServer, db: Database.Database, getSessionId: () => string | undefined) => void,
  options: CreateServerAppOptions,
): ServerApp {
  const port = options.port || 3000;
  const dbPath = options.dbPath || join(process.cwd(), 'data', `${options.serverName.toLowerCase().replace(/\s+/g, '-')}.db`);
  ...
  app.get('/health', createHealthHandler(db, options.serverName, options.getSessionCount));
  ...
  const createMcpServer = (sessionId?: string): McpServer => {
    ...
    registerTools(server, db, () => currentSessionId);
  ...
  app.post('/mcp', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (sessionId && transports[sessionId]) {
        ...
        const server = createMcpServer(transport.sessionId ?? undefined);
  ...
  const start = () => {
    app.listen(port, () => {
      console.log(`[${options.serverName}] MCP server listening on port ${port}`);
