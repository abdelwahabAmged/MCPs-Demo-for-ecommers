...
function baseStyles(): string {
  return `
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .header { background: ${BRAND_COLOR}; color: white; padding: 24px 32px; display: flex; justify-content: space-between; align-items: center; }
    ...
    .print-btn { background: ${ACCENT_COLOR}; color: white; border: none; padding: 8px 20px; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 500; }
    .print-btn:hover { background: ${BRAND_COLOR}; }
...
<div class="header">
  <div><div class="logo">ACME Industrial Supply</div><div class="subtitle">B2B Procurement Portal</div></div>
...
export function renderQuotePage(db: Database.Database, quoteId: string): string | null {
  ...
  const itemRows = items.map(i => `<tr>
    <td>${i.sku}</td><td>${i.name}</td><td class="text-right">${i.qty}</td>
    ...
    <div class="card" style="margin-top:24px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;">
        ...
        <div style="text-align:right;">
          ...
          <span class="badge ${quote.status === 'draft' ? 'badge-open' : 'badge-paid'}" style="margin-top:4px;">${quote.status.toUpperCase()}</span>
        </div>
      </div>
  ...
  return pageShell(`Quote ${quote.quote_id}`, content);
}
...
export function renderTrackingPage(db: Database.Database, orderId: string): string | null {
  ...
  const itemRows = items.map(i =>
    `<tr><td>${i.sku}</td><td>${i.name}</td><td class="text-right">${i.qty}</td><td class="text-right">£${i.unit_price.toFixed(2)}</td><td class="text-right">£${(i.qty * i.unit_price).toFixed(2)}</td></tr>`
  ).join('');
    ...
    <div class="card" style="margin-top:24px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;">
        <div>
          ...
          <div class="text-muted">PO: ${order.po_number} · Ordered: ${order.order_date}</div>
        </div>
  ...
  return pageShell(`Track ${order.order_id}`, content);
}
...
export function renderDashboardPage(db: Database.Database): string {
  ...
  const catBars = Array.from(catSpend.entries()).sort((a, b) => b[1] - a[1]).map(([cat, spend]) => {
    const pct = Math.round(spend / totalCatSpend * 100);
    return `<div style="margin-bottom:8px;">
      <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:2px;"><span>${cat}</span><span>£${spend.toFixed(0)} (${pct}%)</span></div>
  ...
  const orderRows = recentOrders.map(o =>
    `<tr>
      <td><a href="/track/${o.order_id}" style="color:${ACCENT_COLOR};text-decoration:none;font-weight:500;">${o.order_id}</a></td>
  ...
  const invoiceRows = openInvoices.map(i =>
    `<tr>
      <td><a href="/invoice/${i.invoice_id}" style="color:${ACCENT_COLOR};text-decoration:none;">${i.invoice_id}</a></td>
      <td>${i.order_id}</td><td>${i.due_date}</td>
  ...
  const content = `
    <div style="margin-top:24px;margin-bottom:20px;display:flex;justify-content:space-between;align-items:center;">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:20px;">
      ${[
        ...
        { label: 'Outstanding', value: `£${totalOutstanding.toFixed(2)}`, sub: `${openInvoices.length} open invoices` },
  ...
  return pageShell('Account Dashboard', content);
}
...
export function renderCatalogPage(db: Database.Database, category?: string): string {
  ...
  const productCards = products.map(p => {
    ...
    return `<div class="card" style="padding:16px;cursor:pointer;" onclick="this.querySelector('.detail').style.display=this.querySelector('.detail').style.display==='none'?'block':'none'">
      ...
      <div style="display:flex;gap:12px;margin-top:8px;align-items:center;">
        ...
        <span class="badge badge-open" style="font-size:11px;">${p.category}</span>
      </div>
  ...
  const content = `
    <div style="margin-top:24px;margin-bottom:20px;">
      ...
      <div style="margin-bottom:16px;">
        <a href="/catalog" style="display:inline-block;padding:6px 14px;border-radius:20px;font-size:13px;font-weight:500;text-decoration:none;margin:0 4px 4px 0;background:${!category ? ACCENT_COLOR : '#edf2f7'};color:${!category ? 'white' : '#4a5568'};">All (${products.length})</a>
        ${catTabs}
      </div>
    </div>
  ...
  return pageShell('Product Catalog', content);
}
...
export function renderInvoicePage(db: Database.Database, invoiceId: string): string | null {
    ...
    <div class="card" style="margin-top:24px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;">
        <div>
          ...
          <div class="text-muted" style="margin-top:4px;">${invoice.invoice_id}</div>
        </div>
        <div style="text-align:right;">
  ...
  return pageShell(`Invoice ${invoice.invoice_id}`, content);
}
